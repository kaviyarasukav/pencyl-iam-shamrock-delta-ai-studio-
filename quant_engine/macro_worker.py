...
class MacroRegimeAnalyzer:
    ...
    def _update_options_data(self):
        ...
        try:
            ...
            try:
                delta = ccxt.delta({'enableRateLimit': False})
                ticker = delta.fetch_ticker('BTC/USD')
    ...
    def _update_funding_rate(self):
        # Fetch Current Delta Perpetual Funding Rate
        import requests
        try:
            url = "https://api.delta.exchange/v2/products/BTCUSD"
            response = requests.get(url, timeout=5)
            if response.status_code == 200:
                data = response.json()
