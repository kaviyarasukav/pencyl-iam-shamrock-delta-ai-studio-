    ...
    def update(self, snapshot: Dict[str, Any]):
        ...
        for i in range(min(len(raw_bids), self.depth_levels)):
            ...
            if isinstance(bid, dict):
                self.bids[i] = [float(bid.get("p") or 0), float(bid.get("q") or 0)]
            else:
                self.bids[i] = [float(bid[0] or 0), float(bid[1] or 0)]
            
        # Fill asks array (up to depth_levels)
        for i in range(min(len(raw_asks), self.depth_levels)):
            ask = raw_asks[i]
            if isinstance(ask, dict):
                self.asks[i] = [float(ask.get("p") or 0), float(ask.get("q") or 0)]
            else:
                self.asks[i] = [float(ask[0] or 0), float(ask[1] or 0)]
