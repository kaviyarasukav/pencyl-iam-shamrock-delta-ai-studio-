...
const USE_TESTNET_ENV = process.env.DELTA_USE_TESTNET !== "false"; // Default to testnet for safety
...
let deltaBaseUrl: string = useTestnet ? "https://testnet-api.delta.exchange" : "https://api.delta.exchange";

export function loadKeysFromDB() {
  const dbApiKey = getSetting("DELTA_API_KEY", true);
  const dbSecretKey = getSetting("DELTA_SECRET_KEY", true);
  const dbTestnet = getSetting("DELTA_USE_TESTNET", false);

  // Use DB key if it's a non-empty string, otherwise fallback to ENV then null
  activeApiKey =
    dbApiKey && dbApiKey.length > 0
      ? dbApiKey
      : process.env.DELTA_API_KEY || null;
  activeSecretKey =
    dbSecretKey && dbSecretKey.length > 0
      ? dbSecretKey
      : process.env.DELTA_SECRET_KEY || null;
  ...
  deltaBaseUrl = useTestnet ? "https://testnet-api.delta.exchange" : "https://api.delta.exchange";
...
const DELTA_WS_URL = useTestnet ? "wss://testnet-socket.delta.exchange" : "wss://socket.delta.exchange";
const DELTA_PUBLIC_WS_URL = useTestnet ? "wss://testnet-socket.delta.exchange" : "wss://socket.delta.exchange"; // generally Delta doesn't distinguish public/private URLs but standardizes on socket.

// Circuit Breaker & Weight Tracking
let currentWeight = 0;
const WEIGHT_LIMIT = 6000; // Delta standard limit per minute
const CIRCUIT_BREAKER_THRESHOLD = 4800; // 80% of limit
let circuitBreakerTripped = false;

// Create a dedicated axios instance for Delta with timeout
const deltaAxios = axios.create({ validateStatus: () => true, timeout: 30000 });

deltaAxios.interceptors.request.use(config => { if(config.url) { config.url = config.url.replace(/([A-Z]{2,5})USD\b/g, (match, p1) => p1 + 'USDT'); } return config; });
...
deltaAxios.interceptors.request.use(
  (config) => {
    if (circuitBreakerTripped) {
...
deltaAxios.interceptors.response.use(
  (response) => {
    const weightUsed =
...
if (!activeApiKey || !activeSecretKey) {
  console.warn(
    "⚠️ WARNING: DELTA_API_KEY or DELTA_SECRET_KEY is missing from DB and ENV.",
  );
  console.warn(
...
async function spawnEngine() {
  ...
  quantEngine.stdout?.on("data", (data: any) => {
    ...
    if (stdoutBuffer.includes("\\n")) {
      ...
      // The last element is either empty string (if string ended with newline)
  ...
  if (!activeDepthWs.has(upperSymbol)) {
    ...
    ws.on("message", (data: any) => {
       ...
       if (msg.type === "l2_orderbook") {
          const bids = msg.buy?.map((x: any) => [x.limit_price || x.price, x.size]);
          const asks = msg.sell?.map((x: any) => [x.limit_price || x.price, x.size]);
...
function startOptionsStream() {
  ...
  const endpoints = [
    "wss://nbstream.delta.com/eoptions/stream?streams=!trade@arr/!ticker@arr",
    "wss://vstream.delta.com/stream?streams=!trade@arr/!ticker@arr",
  ];

  const currentEndpoint = endpoints[optionsAttempt % endpoints.length];
  console.log(
    `[WS] Connecting to Combined Delta Options Stream (${currentEndpoint})...`,
  );
  ...
  optionsWs.on("error", (err: any) => {
    ...
    if (isWafBlock) {
      ...
      if (optionsAttempt < endpoints.length - 1) {
        ...
        console.log(
          "[WS] All Delta Options endpoints blocked. Falling back to simulated flow.",
        );
...
function signRequest(params: Record<string, any>, secretKeyOverride?: string) {
  ...
  if (!secretToUse) {
    throw new Error("DELTA_SECRET_KEY is missing from configuration");
  }
  ...
  const queryString = Object.keys(params)
    ...
    .map((key) => {
      ...
      if (typeof value === "number") {
        // Use a safe fixed precision that covers all Delta assets (8 decimals is standard)
...
async function fetchInitialBalances(): Promise<boolean> {
  ...
  try {
    ...
    const response = await deltaAxios.get(
      `${deltaBaseUrl}/v2/wallet/balances?${signedQuery}`,
      {
        headers: { "dummy-key": activeApiKey },
      },
    );
    ...
    if (
      ...
      console.warn(
        "⚠️ Cannot authenticate initially. Please provide correct Delta API keys via the UI.",
      );
    } else {
...
async function startUserDataStream() {
  ...
  // Delta has deprecated the User Data Stream via REST /api/v3/userDataStream in 2026.
...
// startOptionsStream();
// if (DELTA_API_KEY && DELTA_SECRET_KEY) {
...
async function startServer() {
  ...
  getInternalExchangeInfo().then(() => {
    console.log("[Boot] Pre-fetched Delta Exchange Info.");
  }).catch((e) => {
  ...
  // The Axios interceptor (`deltaAxios`) already handles Delta rate limits appropriately.
  ...
  app.get("/api/klines/:symbol", async (req, res) => {
    ...
    try {
      ...
      if (typeof interval === "string" && interval.endsWith("h")) resInterval = interval;
      const response = await deltaAxios.get(
        `${deltaBaseUrl}/v2/history/candles?resolution=${resInterval}&symbol=${symbol}&start=${start}&end=${now}`
      );
      if (!response.data || !response.data.success) {
          return res.json([]);
      }
      response.data = Array.isArray(response.data.result) ? response.data.result.map((candle: any) => [
          candle.time * 1000,
  ...
  app.get("/api/orderbook/:symbol", async (req, res) => {
    ...
    try {
      ...
      response.data = {
          ...
          bids: Array.isArray(result.buy) ? result.buy.map((b: any) => [b.price, b.size.toString()]) : [],
          asks: Array.isArray(result.sell) ? result.sell.map((a: any) => [a.price, a.size.toString()]) : []
      };
  ...
  app.get("/api/trades/:symbol", async (req, res) => {
    ...
    try {
      ...
      if (!response.data || !response.data.success) return res.json([]);
      response.data = Array.isArray(response.data.result) ? response.data.result.map((t: any) => ({
          price: t.price,
          qty: t.size.toString(),
  ...
  app.get("/api/account", async (req, res) => {
    ...
    try {
      ...
      const deltaBalances = response.data?.result || [];
      const mappedBalances = deltaBalances.map((b: any) => ({
        asset: b.asset_symbol || b.asset_id?.toString() || "Unknown",
        free: b.available_balance?.toString() || "0",
        locked: (parseFloat(b.balance || "0") - parseFloat(b.available_balance || "0")).toString(),
      }));

      // Stub properties for Delta
      res.json({
        makerCommission: 5,
        takerCommission: 5,
        buyerCommission: 0,
        ...
        permissions: ["MARGIN", "FUTURES"],
        balances: mappedBalances
      });
    } catch (error: any) {
      let errorMessage = "Failed to fetch account details";
      if (error.response?.data?.msg) {
        errorMessage = `Delta Error: ${error.response.data.msg}`;
  ...
  app.get("/api/wallet-summary", async (req, res) => {
    ...
    try {
      ...
      if (!tickers) {
        const tResponse = await deltaAxios.get(`${deltaBaseUrl}/v2/tickers`);
        tickers = tResponse.data.result || [];
      }

      const headers = getDeltaAuthHeaders("GET", "/v2/wallet/balances");
      const response = await deltaAxios.get(`${deltaBaseUrl}/v2/wallet/balances`, { headers });
      const balancesList = response.data?.result || [];

      const portfolio: any[] = [];
      let totalUsdtValue = 0;

      for (const b of balancesList) {
        const asset = b.asset_symbol || "Unknown";
        ...
        if (asset !== "USDT") {
          const pair = Array.isArray(tickers)
            ? tickers.find((t: any) => t.symbol === `${asset}USDT` || t.symbol === `${asset}USD`)
            : null;
          price = pair ? parseFloat(pair.close || pair.lastPrice || "0") : 0;
        }
  ...
  app.get("/api/openOrders/:symbol", async (req, res) => {
    ...
    try {
      ...
      res.json(mappedOrders);    } catch (error: any) {
      console.error(
  ...
  app.get("/api/myTrades/:symbol", async (req, res) => {
    ...
    try {
      ...
      res.json(mapped);    } catch (error: any) {
      console.error(
  ...
  app.delete("/api/order/:symbol/:orderId", async (req, res) => {
    ...
    try {
      ...
      const response = await deltaAxios.delete(
        `${deltaBaseUrl}/v2/orders?${signedQuery}`,
        {
          headers: { "dummy-key": activeApiKey },
        },
      );
  ...
  app.delete("/api/buster-call/:symbol", async (req, res) => {
    ...
    try {
      if (symbol === "GLOBAL") {
        ...
        (async () => {
          try {
            ...
            const openOrdersRes = await deltaAxios.get(
              `${deltaBaseUrl}/v2/orders?${qFetch}`,
              {
                headers: { "dummy-key": killApiKey },
              },
            );
            ...
            await Promise.allSettled(
              symbols.map((sym) => {
                ...
                return deltaAxios
                  .delete(`${deltaBaseUrl}/v2/orders?${qCancel}`, {
                    headers: { "dummy-key": killApiKey },
                  })
                  .catch((e) =>
                    console.error(
            ...
            const accountRes = await deltaAxios.get(`${deltaBaseUrl}/v2/wallet/balances?${accountFetch}`, { headers: { "dummy-key": killApiKey } });
                ...
                if (free > 0 && bal.asset !== "USDT" && bal.asset !== "USDC" && bal.asset !== "BNB" && bal.asset !== "FDUSD") {
                   ...
                   if (symInfo) {
                     ...
                     if (parseFloat(finalQty) > 0) {
                        ...
                        await deltaAxios.post(`${deltaBaseUrl}/v2/orders?${qOrder}`, null, { headers: { "dummy-key": killApiKey } }).catch((e) => console.error(`[BUSTER CALL] Liq Fail ${sym}:`, e.response?.data?.msg || e.message));
                     }
                   }
                }
              })
      ...
      const accountRes = await deltaAxios.get(`${deltaBaseUrl}/v2/wallet/balances?${accountFetch}`, { headers: { "dummy-key": activeApiKey } });
      const exchangeInfo = getCachedData("exchangeInfo", 86400000) as any;
      ...
      if (symInfo) {
        ...
        if (free > 0) {
           ...
           if (parseFloat(finalQty) > 0) {
              ...
              await deltaAxios.post(`${deltaBaseUrl}/v2/orders?${qOrder}`, null, { headers: { "dummy-key": activeApiKey } }).catch((e) => console.error(`[BUSTER CALL] Liq Fail ${symbol}:`, e.response?.data?.msg || e.message));
           }
        }
      }
  ...
  app.post("/api/settings", express.json(), (req, res) => {
    try {
      ...
      if (apiKey !== undefined) {
        setSetting("DELTA_API_KEY", apiKey, true);
        keysChanged = true;
      }
      if (secretKey !== undefined) {
        setSetting("DELTA_SECRET_KEY", secretKey, true);
        keysChanged = true;
      }
      if (useTestnet !== undefined) {
        setSetting("DELTA_USE_TESTNET", useTestnet ? "true" : "false", false);
        keysChanged = true;
      }
  ...
  const discoverAssets = async () => {
    ...
    try {
      ...
      const tickers = resultData.filter((t: any) =>
  ...
  const server = app.listen(listenPort, "0.0.0.0", () => {
    ...
    if (activeApiKey && activeSecretKey) {
      ...
      deltaAxios
        .get(
          `${deltaBaseUrl}/v2/orders?timestamp=${Date.now()}&signature=${signature}`,
          {
            headers: { "dummy-key": activeApiKey },
          },
        )
        .then((res) => {
          if (res.data && res.data.length > 0) {
            ...
            symbols.forEach((sym) => {
              ...
              deltaAxios
                .delete(
                  `${deltaBaseUrl}/v2/orders?${q}&signature=${sig}`,
                  {
                    headers: { "dummy-key": activeApiKey },
                  },
                )
...
function gracefulShutdown() {
  ...
  if (activeApiKey && activeSecretKey) {
    ...
    const promises = symbolsToCancel.map((symbol) => {
      ...
      return deltaAxios
        .delete(
          `${deltaBaseUrl}/v2/orders?symbol=${symbol}&timestamp=${timestamp}&signature=${signature}`,
          {
            headers: { "dummy-key": activeApiKey },
          },
        )
