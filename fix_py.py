...
content = open("quant_engine/main.py").read()
...
old_sub = """                streams = [f"{s.replace('/', '').lower()}@depth20@100ms" for s in new_symbols] + [f"{s.replace('/', '').lower()}@aggTrade" for s in new_symbols]
                subscribe_msg = {
                    "method": "SUBSCRIBE",
                    "params": streams,
                    "id": 1
...
open("quant_engine/main.py", "w").write(content)
print("done")
