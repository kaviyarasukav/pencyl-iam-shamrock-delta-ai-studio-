...
class ApiClient {
  ...
  public deltaBaseUrl: string = 'https://api.delta.exchange'; // Defaults
  ...
  constructor() {
    ...
    // Request interceptor for robust error handling wrapper and symbol mapping
    this.api.interceptors.request.use(
      (config) => {
        // Symbol Mapping: Translate USD standard pairs to Delta USDT format
        if (config.url) {
           config.url = config.url.replace(/([A-Z]{2,5})USD\b/, '$1USDT');
           
           // Rewrite to V2 Delta natively to bypass Node proxy for high-volume market data
    ...
    // Response interceptor to manage rate limits, invalid keys, and schema validation gracefully
    this.api.interceptors.response.use(
      (response: AxiosResponse) => {
        // Schema Validation Layer
        const url = response.config.url || '';
        
        // Validation for Delta v2 API formats
        if (url.includes('/trades')) {
            if (response.data && response.data.success !== undefined && !Array.isArray(response.data)) {
                if (response.data.success === false || response.data.error) {
                    ...
                    // Normalize standard delta format wrapper out
        ...
        if (url.includes('/orderbook')) {
            if (response.data && response.data.success === false) {
                 ...
                 // Convert delta v2 orderbook exactly as frontend expects
                 const result = response.data.result;
                 response.data = {
                    lastUpdateId: result.last_updated_at || Date.now(),
                    bids: result.buy ? result.buy.map((b: any) => [b.limit_price || b.price, b.size]) : [],
                    asks: result.sell ? result.sell.map((a: any) => [a.limit_price || a.price, a.size]) : []
                 };
            }
        }
      ...
      async (error: AxiosError) => {
        ...
        if (error.response) {
          ...
          if (status === 400 && error.response.config.url?.includes('delta.exchange/v2/')) {
             console.warn(`[Schema/API Warning] Delta API returned + ${status}. Config url: ${error.response.config.url}, data:`, error.response.data);
