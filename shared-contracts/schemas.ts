...
export const StrategySignalSchema = Joi.object<StrategySignal>({
  ...
  symbol: Joi.string().required(),
  action: Joi.string().valid('BUY', 'SELL', 'CLOSE_LONG', 'CLOSE_SHORT', 'HALT', 'RESUME', 'HALT_ALL').required(),
  order_type: Joi.string().valid('MARKET', 'LIMIT', 'LIMIT_MAKER', 'STOP_LOSS', 'STOP_LOSS_LIMIT', 'TAKE_PROFIT', 'TAKE_PROFIT_LIMIT', 'OCO', 'LIMIT_CHASE', 'TWAP', 'CANCEL').required(),
  ...
  stop_loss: Joi.alternatives().try(Joi.number(), Joi.string()).optional(),
  take_profit: Joi.alternatives().try(Joi.number(), Joi.string()).optional(),
  position_size_usd: Joi.alternatives().try(Joi.number(), Joi.string()).optional(),
...
export const ExecutionOrderSchema = Joi.object<ExecutionOrder>({
  ...
  type: Joi.string().valid('MARKET', 'LIMIT', 'LIMIT_MAKER', 'STOP_LOSS', 'STOP_LOSS_LIMIT', 'TAKE_PROFIT', 'TAKE_PROFIT_LIMIT', 'OCO', 'CANCEL', 'CANCEL_REPLACE', 'LIMIT_CHASE', 'TWAP').required(),
  ...
  metadata: Joi.object().optional(),
  algo: Joi.string().valid('TWAP', 'VWAP', 'PEG_BBO').optional(),
  ...
  isShadow: Joi.boolean().optional(),
  cancelReplaceClientOrderId: Joi.string().optional()
});
